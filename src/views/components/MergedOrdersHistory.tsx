      ...
      {(!mergedOrders || mergedOrders.length === 0) ? (
        ...
        <div className="space-y-4">
          {mergedOrders.map((order) => {
            ...
            <div
              key={order.id}
              className="bg-white dark:bg-stone-800 rounded-xl shadow-md p-6 border-2 border-green-200 hover:shadow-lg transition-shadow"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <CheckCircleIcon className="w-6 h-6 text-green-600" />
                    <h3 className="text-xl font-bold text-stone-800 dark:text-stone-100">Đơn #{shortId}</h3>
                  </div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">{order.totalItems ?? order.total_items ?? 0} món</p>
                </div>
              </div>

              {/* Items */}
              <div className="bg-stone-50 rounded-lg p-4">
                <h4 className="font-semibold text-stone-700 dark:text-stone-200 mb-3">Chi tiết đơn hàng:</h4>
