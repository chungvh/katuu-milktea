    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="bg-white dark:bg-stone-800 rounded-xl shadow-md p-6 mb-8">
          <h2 className="text-xl font-bold text-stone-800 dark:text-stone-100 mb-6">Doanh Thu 7 Ngày Gần Nhất</h2>
          <div className="space-y-4">
            {dailyRevenue.map((day, index) => (
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium text-stone-700 dark:text-stone-200 w-24">{day.date}</span>
                  <span className="text-stone-500 dark:text-stone-400">{day.orders} đơn</span>
                  <span className="font-semibold text-amber-600 dark:text-amber-400 w-32 text-right">{formatVND(day.revenue)}</span>
                </div>
          ...
          <div className="bg-white dark:bg-stone-800 rounded-xl shadow-md p-6">
            ...
            <div className="space-y-3">
              {productStats.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-stone-500 dark:text-stone-400">{stat.totalOrders} đơn</p>
                      </div>
                    </div>
                ...
                <p className="text-center text-stone-500 dark:text-stone-400 py-8">Chưa có dữ liệu</p>
              )}
            </div>
          </div>
              ...
              {categoryStats.length > 0 ? (
                categoryStats.map((stat, index) => (
                  <div key={stat.category.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-stone-700 dark:text-stone-200">{stat.category.name}</span>
                      <div className="flex items-center space-x-3">
                        <span className="text-sm text-stone-500 dark:text-stone-400">{stat.totalOrders} đơn</span>
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="flex-1 bg-stone-100 rounded-full h-2 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${index === 0 ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
        ...
        <div className="bg-white dark:bg-stone-800 rounded-xl shadow-md p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {toppingStats.length > 0 ? (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-stone-500 dark:text-stone-400">{stat.count} lần</p>
                    </div>
                  </div>
                  <div className="text-right">
              ...
              <p className="col-span-3 text-center text-stone-500 dark:text-stone-400 py-8">Chưa có dữ liệu</p>
            )}
          </div>
        </div>
        ...
        {/* Empty State */}
        {totalOrders === 0 && (
          <div className="bg-white dark:bg-stone-800 rounded-xl shadow-md p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-bold text-stone-800 dark:text-stone-100 mb-2">Chưa Có Đơn Hàng</h3>
